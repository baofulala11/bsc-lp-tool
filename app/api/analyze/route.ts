...
async function fetchTokenPools(tokenAddress: string): Promise<PoolData[]> {
...
async function fetchTopPositions(pool: PoolData) {
...
export async function POST(request: Request) {
  try {
    ...
    const poolList = await fetchTokenPools(tokenAddress);
    
    if (poolList.length === 0) {
      return NextResponse.json({ error: 'No V3 pools found' }, { status: 404 });
    }
    ...
    return NextResponse.json(results.filter(r => r.positions && r.positions.length > 0));
  } catch (error) {
    console.error(error);
